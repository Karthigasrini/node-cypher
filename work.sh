...
# Function: to start, stop and restart java-cypher.
...
JAR_FILE_PATH="./build/libs/java-cypher.jar"
PID_FILE_PATH="java-cypher.pid"
LOG_FILE_PATH="java-cypher.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-cypher."
        else
            echo "Failed to start java-cypher."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-cypher."
        else
            echo "Failed to stop java-cypher."
        fi
    ;;
    restart)
