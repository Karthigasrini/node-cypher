...
Having a tailored actuator is key to building a java-cypher-based customized public chain. This article illustrates how to develop a java-cypher-based `SumActuator`.
...
Currently, contracts supported by java-cypher are defined under `src/main/protos/core/contract` directory in protocol module. First creating a `math_contract.proto` file under this directory and declaring `SumContract`. You can also implement any mathematical calculation you want, such as `Subtraction`.
...
Meanwhile, register the new contract type in `Transaction.Contract.ContractType` emuneration within the `src/main/protos/core/Cypher.proto` file. Important data structures, such as transactions, accounts and blocks, are defined in the `Cypher.proto` file:
...
```
At last, recompile the modified proto files. Compiling the java-cypher project directly will compile the proto files as well, `protoc` command is also supported.

*Currently, java-cypher uses protoc v3.4.0. Please keep the same version when compiling by `protoc` command.*
...
For now, the default Actuator supported by java-cypher is located in `org.cypher.core.actuator`. Creating `SumActuator` under this directory:
