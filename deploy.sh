...
if [[ "$TRAVIS_BRANCH" = "develop_1" || "$TRAVIS_BRANCH" = "master_1" ]];then
    ...
    stest_server=""
    docker_num_in_145=`ssh -p 22008 -t java-cypher@47.93.42.145 'docker ps -a | wc -l'`
    docker_num_in_145=`echo $docker_num_in_145 | tr -d "\r"`
    docker_num_in_60=`ssh -p 22008 -t java-cypher@47.93.18.60 'docker ps -a | wc -l'`
    ...
    ssh java-cypher@$stest_server -p 22008 $change_branch_CMD
    `ssh java-cypher@$stest_server -p 22008 sh /data/workspace/docker_workspace/do_stest.sh >$stestlogname 2>&1` &
