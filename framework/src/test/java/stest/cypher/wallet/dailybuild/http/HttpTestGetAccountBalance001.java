package stest.cypher.wallet.dailybuild.http;
...
import stest.cypher.wallet.common.client.utils.HttpMethed;
...
public class HttpTestGetAccountBalance001 {
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Get burn cyp by http")
  public void test03GetBurnCyp() {
    ...
    HttpMethed.waitToProduceOneBlock(httpnode);
    final Long beforeBurnCypAmount = HttpMethed.getBurnCyp(httpnode);
    ECKey ecKey3 = new ECKey(Utils.getRandom());
    ...
    HttpMethed.waitToProduceOneBlockFromSolidity(httpnode, httpSolidityNode);
    Long afterBurnCypAmount = HttpMethed.getBurnCyp(httpnode);
    logger.info(afterBurnCypAmount + "  :   " + beforeBurnCypAmount);
    Assert.assertTrue(afterBurnCypAmount - beforeBurnCypAmount == 100000L);

    Assert.assertEquals(afterBurnCypAmount, HttpMethed.getBurnCypFromSolidity(httpSolidityNode));
    Assert.assertEquals(afterBurnCypAmount, HttpMethed.getBurnCypFromPbft(httpPbftNode));
  }
