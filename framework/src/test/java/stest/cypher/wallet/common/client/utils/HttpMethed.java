...
public class HttpMethed {
  ...
  public static HttpResponse assetIssue(
    ...
    try {
      ...
      userBaseObj2.addProperty("cyp_num", cypNum);
  ...
  public static HttpResponse freezeBalance(
    ...
    try {
      ...
      if (resourceCode == 2) {
        userBaseObj2.addProperty("resource", "CYPHER_POWER");
      }
  ...
  /** constructor. */
  public static Long getBurnCyp(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/wallet/getburncyp";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
    ...
    responseContent = HttpMethed.parseResponseContent(response);
    return responseContent.getLong("burnCypAmount");
  }

  /** constructor. */
  public static Long getBurnCypFromSolidity(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletsolidity/getburncyp";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  /** constructor. */
  public static Long getBurnCypFromPbft(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletpbft/getburncyp";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static HttpResponse sendShieldCoin(
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        response =
            HttpMethed.getMerkleTreeVoucherInfo(httpNode, noteTx.getCypId(), noteTx.getIndex(), 1);
        responseContent = HttpMethed.parseResponseContent(response);
        ...
        List shieldedSpends =
            Lists.newArrayList(
                new JSONObjectWarp()
                    ...
                    .put("alpha", ByteArray.toHexString(org.cypher.core.zen.note.Note.generateR()))
  ...
  public static List<ShieldNoteInfo> scanNoteByOvkFromPbft(
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        noteTx.setCypId(HttpMethed.parseStringContent(noteTxs).getString("txid"));
  ...
  public static HttpResponse sendShieldCoinWithoutAsk(
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        logger.info("noteTx.getCypId():" + noteTx.getCypId());
        HttpMethed.response =
            HttpMethed.getMerkleTreeVoucherInfoFromSolidity(
                httpSolidityNode, noteTx.getCypId(), noteTx.getIndex(), 1);
        ...
        HttpMethed.response =
            HttpMethed.getMerkleTreeVoucherInfoFromPbft(
                httpPbftNode, noteTx.getCypId(), noteTx.getIndex(), 1);
        HttpMethed.responseContent = HttpMethed.parseResponseContent(HttpMethed.response);
        HttpMethed.printJsonContent(responseContent);
        ...
        alpha = org.cypher.core.zen.note.Note.generateR();
