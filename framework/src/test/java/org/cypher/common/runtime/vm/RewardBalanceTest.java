  ...
  */
  ...
  public void testRewardBalance()
    ...
    Transaction cyp = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] factoryAddress = WalletUtil.generateContractAddress(cyp);
    ...
    cyp = TvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    InternalTransaction rootInternalTransaction = new InternalTransaction(cyp,
        InternalTransaction.CypType.CYP_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
    ProgramInvoke programInvoke = ProgramInvokeFactory
        .createProgramInvoke(InternalTransaction.CypType.CYP_CONTRACT_CALL_TYPE,
            InternalTransaction.ExecutorType.ET_PRE_TYPE, cyp,
    ...
    cyp = TvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    rootInternalTransaction = new InternalTransaction(cyp,
        InternalTransaction.CypType.CYP_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
