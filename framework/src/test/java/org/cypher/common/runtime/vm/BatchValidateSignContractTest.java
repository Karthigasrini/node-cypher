...
import org.cypher.core.vm.PrecompiledContracts.BatchValidateSign;
...
public class BatchValidateSignContractTest {
  ...
  public void staticCallTest() {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      if (i == 13) {
        addresses
            .add(StringUtil.encode58Check(TransactionTrace.convertToCypherAddress(new byte[20])));
      } else {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      if (i % 5 == 0) {
        addresses.add(StringUtil.encode58Check(TransactionTrace
            .convertToCypherAddress(new byte[20])));
        signatures.add(Hex.toHexString(DataWord.ONE().getData()));
      } else {
        addresses.add(StringUtil.encode58Check(key.getAddress()));
        signatures.add(Hex.toHexString(sign));
