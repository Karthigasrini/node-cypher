...
import static org.cypher.protos.Protocol.Transaction.Result.contractResult;
import static org.cypher.protos.Protocol.Transaction.Result.contractResult.REVERT;
...
import stest.cypher.wallet.common.client.utils.DataWord;
...
public class VoteTest {
  ...
  private static final long cyp_precision = 1_000_000L;
  ...
  private byte[] deployContract(String contractName, String abi, String code) throws Exception {
    Protocol.Transaction cyp = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, owner, abi, code, value, fee, 80,
        null, 1_000_000L);
    cyp = cyp.toBuilder().setRawData(
        cyp.getRawData().toBuilder().setTimestamp(System.currentTimeMillis()).build()).build();
    byte[] contractAddr = WalletUtil.generateContractAddress(cyp);
    //String contractAddrStr = StringUtil.encode58Check(contractAddr);
    TransactionCapsule cypCap = new TransactionCapsule(cyp);
  ...
  private TVMTestResult triggerContract(byte[] contractAddr,
    ...
    TransactionCapsule cypCap = new TransactionCapsule(
  ...
  public void testVote() throws Exception {
    ...
    // cycle-1
    {
      // query cypher power, is zero
      triggerContract(voteContract, SUCCESS, getEqualConsumer(0),
          queryTotalVoteCountMethod, voteContractStr);

      // get cypher power: bandwidth
      triggerContract(voteContract, SUCCESS, null,
          freezeMethod, voteContractStr, freezeUnit, 0);
      // get cypher power: energy
      triggerContract(voteContract, SUCCESS, null,
          freezeMethod, voteContractStr, freezeUnit, 1);

      // query cypher power, not zero
      long totalCypherPower = 2 * freezeUnit / cyp_precision;
      triggerContract(voteContract, SUCCESS, getEqualConsumer(totalCypherPower),
          queryTotalVoteCountMethod, voteContractStr);
  ...
  public void testRewardAlgorithmNo1() throws Exception {
    ...
    // cycle-1
    {
      // freeze balance to get cypher power
  ...
  private void voteWitness(byte[] contract,
                           ...
                           List<Long> cypherPowerList) throws Exception {
    triggerContract(contract, SUCCESS, getEqualConsumer(1),
        voteMethod, witnessList, cypherPowerList);
  }
