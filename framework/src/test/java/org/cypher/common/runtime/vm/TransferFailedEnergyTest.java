...
import org.cypher.common.runtime.TVMTestResult;
import org.cypher.common.runtime.TvmTestUtils;
...
public class TransferFailedEnergyTest extends VMTestBase {
  ...
  contract EnergyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferCypInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendCypInsufficientBalance() payable public{
          msg.sender.send(10);
      }
      ...
      function testCallCypInsufficientBalance(address payable caller) public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }

      function testCreateCypInsufficientBalance() payable public {
          (new Caller).value(10)();
      }

      // NonexistentTarget
      function testTransferCypNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testCallCypNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferCypSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendCypSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }
  ...
  contract EnergyOfTransferFailedTest {
      ...
      function testCallCypInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferCypNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallCypNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  TestCase[] testCasesAfterAllowTvmConstantinop = {
      new TestCase("testTransferCypSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSendCypSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testTransferCypNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testCallCypNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
  };
  TestCase[] testCasesBeforeAllowTvmConstantinop = {
      new TestCase("testTransferCypSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSendCypSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSuicideNonexistentTarget(address)",
  ...
  TestCase[] testCasesInsufficientBalance = {
      new TestCase("testTransferCypInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testSendCypInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.SUCCESS),
      new TestCase("testCreateCypInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testCallCypInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
  ...
  public void testTransferFailedAfterAllowTvmConstantinopl()
    ...
    // deploy contract
    Transaction cyp = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, value, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithSufficientBalance = WalletUtil.generateContractAddress(cyp);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(cyp, rootDeposit, null);
    ...
    cyp = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, 0, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithoutBalance = WalletUtil.generateContractAddress(cyp);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(cyp, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
