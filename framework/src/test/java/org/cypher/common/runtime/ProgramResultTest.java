...
import static org.cypher.core.capsule.utils.TransactionUtil.buildTransactionInfoInstance;
import static org.cypher.core.utils.TransactionUtil.generateContractAddress;
...
public class ProgramResultTest {
  ...
  public void successAndFailResultTest()
    ...
    TransactionTrace traceSuccess = TvmTestUtils
        .processTransactionAndReturnTrace(cyp1, deposit, null);
    ...
    checkTransactionInfo(traceSuccess, cyp1, null, internalTransactionsList);
    ...
    Transaction cyp2 = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), aContract,
            triggerData2, 0, 100000000);
    TransactionTrace traceFailed = TvmTestUtils
        .processTransactionAndReturnTrace(cyp2, deposit, null);
    runtime = traceFailed.getRuntime();

    byte[] bContract2 =
        generateContractAddress(new TransactionCapsule(cyp2).getTransactionId().getBytes(), 0);
    ...
    checkTransactionInfo(traceFailed, cyp2, null, internalTransactionsListFail);
  }
  ...
  public void timeOutFeeTest()
    ...
    TransactionInfoCapsule cypInfoCapsule =
        buildTransactionInfoInstance(new TransactionCapsule(cyp1), null, traceSuccess);
    Assert.assertEquals(cypInfoCapsule.getFee(), 12705900L);
    Assert.assertEquals(cypInfoCapsule.getPackingFee(), 0L);
    ...
    cypInfoCapsule =
        buildTransactionInfoInstance(new TransactionCapsule(cyp1), null, traceSuccess);
    Assert.assertEquals(cypInfoCapsule.getFee(), 12705900L);
    Assert.assertEquals(cypInfoCapsule.getPackingFee(), 12705900L);
  ...
  public void suicideResultTest()
    ...
    Transaction cyp = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), suicideContract,
            triggerData1, 0, 100000000);
    TransactionTrace trace = TvmTestUtils.processTransactionAndReturnTrace(cyp, deposit, null);
    ...
    Assert.assertEquals(internalTransactionsList.get(0).isRejected(), false);
    checkTransactionInfo(trace, cyp, null, internalTransactionsList);
  }
  ...
  public void checkTransactionInfo(TransactionTrace trace, Transaction cyp, BlockCapsule block,
      List<InternalTransaction> internalTransactionsList) {
    TransactionInfoCapsule cypInfoCapsule =
        buildTransactionInfoInstance(new TransactionCapsule(cyp), null, trace);
    List<Protocol.InternalTransaction> internalTransactionListFromProtocol = cypInfoCapsule
