...
public class TransactionStoreTest {
  ...
  public void getTransactionTest() throws BadItemException, ItemNotFoundException {
    ...
    final TransactionStore cypStore = chainBaseManager.getTransactionStore();
    ...
    cyp.setBlockNum(blockCapsule.getNum());
    blockStore.put(blockCapsule.getBlockId().getBytes(), blockCapsule);
    cypStore.put(cyp.getTransactionId().getBytes(), cyp);
    Assert.assertEquals("Get transaction is error",
        cypStore.get(cyp.getTransactionId().getBytes()).getInstance(), cyp.getInstance());
    ...
    tc =
        TransferContract.newBuilder()
            ...
            .setToAddress(ByteString.copyFromUtf8("bbb"))
            .build();
    cyp = new TransactionCapsule(tc, ContractType.TransferContract);
    Assert.assertNull(cypStore.get(cyp.getTransactionId().getBytes()));
  ...
  public void getUncheckedTransactionTest() {
    ...
    cypStore.put(cyp.getTransactionId().getBytes(), cyp);
    Assert.assertEquals("Get transaction is error",
        cypStore.getUnchecked(cyp.getTransactionId().getBytes()).getInstance(), cyp.getInstance());
    ...
    tc =
        TransferContract.newBuilder()
            ...
            .build();
    cyp = new TransactionCapsule(tc, ContractType.TransferContract);
    Assert.assertNull(cypStore.getUnchecked(cyp.getTransactionId().getBytes()));
