...
public class NullifierStoreTest {
  ...
  private static final byte[] CYP_TWO = randomBytes(32);
  private static final byte[] CYP_TWO_NEW = randomBytes(32);
  ...
  public static void init() {
    ...
    nullifier2 = new BytesCapsule(CYP_TWO);
    nullifier2New = new BytesCapsule(CYP_TWO_NEW);
  ...
  public void putAndGet() {
    ...
    Assert.assertArrayEquals("putAndGet2", nullifier, CYP_TWO);

    nullifierStore.put(NULLIFIER_TWO, nullifier2New);
    nullifier = nullifierStore.get(NULLIFIER_TWO).getData();
    Assert.assertArrayEquals("putAndGet2New", nullifier, CYP_TWO_NEW);
  }
