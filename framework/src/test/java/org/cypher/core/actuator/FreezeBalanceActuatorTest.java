...
import static org.cypher.core.config.Parameter.ChainConstant.TRANSFER_FEE;
...
public class FreezeBalanceActuatorTest {
  ...
  private Any getContractForCypherPower(String ownerAddress, long frozenBalance, long duration) {
    return Any.pack(
        FreezeBalanceContract.newBuilder()
            .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAddress)))
            .setFrozenBalance(frozenBalance)
            .setFrozenDuration(duration)
            .setResource(ResourceCode.CYPHER_POWER)
            .build());
  }
  ...
  public void lessThan1CypTest() {
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("frozenBalance must be more than 1CYP", e.getMessage());
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    }
  }
  ...
  @Test
  public void testFreezeBalanceForEnergyWithoutOldCypherPowerAfterNewResourceModel() {
    long frozenBalance = 1_000_000_000L;
    long duration = 3;
    FreezeBalanceActuator actuator = new FreezeBalanceActuator();
    ...
    try {
      ...
      Assert.assertEquals(-1L, owner.getInstance().getOldCypherPower());
      Assert.assertEquals(0L, owner.getAllCypherPower());
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
  ...
  @Test
  public void testFreezeBalanceForEnergyWithOldCypherPowerAfterNewResourceModel() {
    long frozenBalance = 1_000_000_000L;
    long duration = 3;
    FreezeBalanceActuator actuator = new FreezeBalanceActuator();
    ChainBaseManager chainBaseManager = dbManager.getChainBaseManager();
    ...
    try {
      ...
      Assert.assertEquals(100L, owner.getInstance().getOldCypherPower());
      Assert.assertEquals(100L, owner.getAllCypherPower());
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
  ...
  @Test
  public void testFreezeBalanceForCypherPowerWithOldCypherPowerAfterNewResourceModel() {
    long frozenBalance = 1_000_000_000L;
    long duration = 3;
    FreezeBalanceActuator actuator = new FreezeBalanceActuator();
    ChainBaseManager chainBaseManager = dbManager.getChainBaseManager();
    chainBaseManager.getDynamicPropertiesStore().saveAllowNewResourceModel(1L);
    actuator.setChainBaseManager(chainBaseManager)
        .setAny(getContractForCypherPower(OWNER_ADDRESS, frozenBalance, duration));
    TransactionResultCapsule ret = new TransactionResultCapsule();
    ...
    try {
      ...
      Assert.assertEquals(100L, owner.getInstance().getOldCypherPower());
      Assert.assertEquals(frozenBalance + 100L, owner.getAllCypherPower());
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
