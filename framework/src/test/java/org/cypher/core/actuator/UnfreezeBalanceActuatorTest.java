...
import org.cypher.protos.contract.AssetIssueContractOuterClass;
import org.cypher.protos.contract.BalanceContract.UnfreezeBalanceContract;
import org.cypher.protos.contract.Common.ResourceCode;
...
public class UnfreezeBalanceActuatorTest {
  ...
  private Any getContractForCypherPower(String ownerAddress) {
    return Any.pack(UnfreezeBalanceContract.newBuilder()
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAddress)))
        .setResource(ResourceCode.CYPHER_POWER).build());
  }
  ...
  public void testUnfreezeBalanceForBandwidth() {
    ...
    Assert.assertEquals(accountCapsule.getCypherPower(), frozenBalance);
    ...
    try {
      ...
      Assert.assertEquals(owner.getCypherPower(), 0L);
  ...
  public void testUnfreezeDelegatedBalanceForBandwidth() {
    ...
    Assert.assertEquals(frozenBalance, owner.getCypherPower());
    ...
    Assert.assertEquals(0L, receiver.getCypherPower());
    ...
    try {
      ...
      Assert.assertEquals(0L, ownerResult.getCypherPower());
  ...
  @Test
  public void testUnfreezeBalanceForEnergyWithOldCypherPowerAfterNewResourceModel() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setOldCypherPower(frozenBalance);
    ...
    try {
      ...
      Assert.assertEquals(owner.getInstance().getOldCypherPower(), -1L);
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
  ...
  @Test
  public void testUnfreezeBalanceForEnergyWithoutOldCypherPowerAfterNewResourceModel() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    dbManager.getDynamicPropertiesStore().saveAllowNewResourceModel(1L);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setOldCypherPower(-1L);
    accountCapsule.addVotes(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)), 100L);
  ...
  @Test
  public void testUnfreezeBalanceForCypherPowerWithOldCypherPowerAfterNewResourceModel() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    dbManager.getDynamicPropertiesStore().saveAllowNewResourceModel(1L);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setFrozenForCypherPower(frozenBalance, now);
    accountCapsule.addVotes(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)), 100L);

    dbManager.getAccountStore().put(accountCapsule.createDbKey(), accountCapsule);
    UnfreezeBalanceActuator actuator = new UnfreezeBalanceActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContractForCypherPower(OWNER_ADDRESS));
    TransactionResultCapsule ret = new TransactionResultCapsule();
  ...
  @Test
  public void testUnfreezeBalanceForCypherPowerWithOldCypherPowerAfterNewResourceModelError() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    dbManager.getDynamicPropertiesStore().saveAllowNewResourceModel(1L);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setFrozenForCypherPower(frozenBalance, now + 100000000L);
    accountCapsule.addVotes(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)), 100L);
