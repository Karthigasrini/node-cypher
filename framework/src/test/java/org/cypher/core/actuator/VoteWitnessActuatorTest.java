...
public class VoteWitnessActuatorTest {
  ...
  public void balanceNotSufficient() {
    ...
    try {
      actuator.validate();
      actuator.execute(ret);
      fail("The total number of votes[" + 1000000 + "] is greater than the cypherPower["
          + balanceNotSufficientCapsule.getCypherPower() + "]");
    } catch (ContractValidateException e) {
      Assert.assertEquals(0, dbManager.getAccountStore()
          .get(ByteArray.fromHexString(OWNER_ADDRESS_BALANCENOTSUFFICIENT)).getVotesList().size());
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert
          .assertEquals("The total number of votes[" + 1000000 + "] is greater than the cypherPower["
              + balanceNotSufficientCapsule.getCypherPower() + "]", e.getMessage());
      maintenanceManager.doMaintenance();
      WitnessCapsule witnessCapsule = dbManager.getWitnessStore()
          .get(StringUtil.hexString2ByteString(WITNESS_ADDRESS).toByteArray());
      Assert.assertEquals(10, witnessCapsule.getVoteCount());
  ...
  @Test
  public void voteWitnessWithoutEnoughOldCypherPowerAfterNewResourceModel() {
  ...
  @Test
  public void voteWitnessWithOldCypherPowerAfterNewResourceModel() {
    ...
    try {
      ...
      owner =
          dbManager.getAccountStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
      Assert.assertEquals(2000000L, owner.getInstance().getOldCypherPower());
    } catch (ContractValidateException e) {
      e.printStackTrace();
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
  ...
  @Test
  public void voteWitnessWithOldAndNewCypherPowerAfterNewResourceModel() {
    ...
    AccountCapsule owner =
        dbManager.getAccountStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
    owner.setFrozenForEnergy(2000000L,0L);
    owner.setFrozenForCypherPower(1000000L,0L);
    dbManager.getAccountStore().put(ByteArray.fromHexString(OWNER_ADDRESS),owner);
    ...
    try {
      ...
      owner =
          dbManager.getAccountStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
      Assert.assertEquals(3000000L, owner.getAllCypherPower());
      Assert.assertEquals(2000000L, owner.getInstance().getOldCypherPower());
      Assert.assertEquals(1000000L, owner.getInstance().getCypherPower().getFrozenBalance());
    } catch (ContractValidateException e) {
      e.printStackTrace();
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
  ...
  @Test
  public void voteWitnessWithoutEnoughOldAndNewCypherPowerAfterNewResourceModel() {
