...
public class ConcurrentHashMapTest {
  ...
  public void testHandleBlockHash() {
    ...
    Map<String, BlockFilterAndResult> conMap = CypherJsonRpcImpl.getBlockFilter2ResultFull();
      ...
      public void run() {
        ...
        for (int i = 1; i <= times; i++) {
          ...
          for (int j = 1 + (i - 1) * eachCount; j <= i * eachCount; j++) {
            ...
            CypherJsonRpcImpl.handleBLockFilter(blockFilterCapsule);
          }
          try {
    ...
    Thread getThread1 = new Thread(new Runnable() {
      public void run() {
        for (int t = 1; t <= times * 2; t++) {
          ...
          for (int k = 0; k < 5; k++) {
            try {
              Object[] blockHashList = CypherJsonRpcImpl.getFilterResult(String.valueOf(k), conMap,
                  CypherJsonRpcImpl.getEventFilter2ResultFull());
