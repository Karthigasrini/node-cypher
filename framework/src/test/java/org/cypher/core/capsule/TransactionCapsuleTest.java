...
public class TransactionCapsuleTest {
  ...
  public void addSign() {
    ...
    Transaction.Builder cypBuilder = Transaction.newBuilder();
    ...
    cypBuilder.setRawData(rawBuilder);
    AccountStore accountStore = dbManager.getAccountStore();
    TransactionCapsule transactionCapsule = new TransactionCapsule(cypBuilder.build());
    //Accout not exist
    try {
  ...
  public void validateSignature0() {
    ...
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    cypBuilder.clearSignature().addSignature(test);
    try {
      TransactionCapsule.validateSignature(cypBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    new Random().nextBytes(rand);
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    cypBuilder.clearSignature().addSignature(ByteString.copyFrom(rand));
    try {
      TransactionCapsule.validateSignature(cypBuilder.build(), hash,
          accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertEquals(e.getMessage(), "Header byte out of range: 35");
    ...
    cypBuilder.clearSignature().addAllSignature(sign21_11);
    try {
      TransactionCapsule.validateSignature(cypBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
    ...
    cypBuilder.clearSignature().addAllSignature(sign21_11_22_23);
    try {
      TransactionCapsule
          .validateSignature(cypBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    cypBuilder.clearSignature().addAllSignature(sign21_22_21);
    try {
      TransactionCapsule
          .validateSignature(cypBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    ...
    cypBuilder.clearSignature().addAllSignature(sign21);
    try {
    ...
    cypBuilder.clearSignature().addAllSignature(sign21_22);
    try {
      boolean result = TransactionCapsule
          .validateSignature(cypBuilder.build(), hash, accountStore);
    ...
    cypBuilder.clearSignature().addAllSignature(sign21_22_23);
    try {
      boolean result = TransactionCapsule
          .validateSignature(cypBuilder.build(), hash, accountStore);
      Assert.assertTrue(result);
  ...
  public void validateSignature1() {
    ...
    ArrayList<ByteString> sign = sign(prikeys, Sha256Hash.hash(rawBuilder.build().toByteArray()));
    cypBuilder.addAllSignature(sign);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());

    // no contract
    prikeys.clear();
    prikeys.add(ByteArray.fromHexString(KEY_21));
    cypBuilder = Transaction.newBuilder();
    ...
    cypBuilder = transactionCapsule.getInstance().toBuilder();
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    cypBuilder.clearSignature();
    cypBuilder.addSignature(test);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    cypBuilder.clearSignature();
    cypBuilder.addSignature(ByteString.copyFrom(rand));
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11 = sign(prikeys, hash);
    cypBuilder.clearSignature();
    cypBuilder.addAllSignature(sign21_11);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11_22_23 = sign(prikeys, hash);
    cypBuilder.clearSignature();
    cypBuilder.addAllSignature(sign21_11_22_23);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22_21 = sign(prikeys, hash);
    cypBuilder.clearSignature();
    cypBuilder.addAllSignature(sign21_22_21);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21 = sign(prikeys, hash);
    cypBuilder.clearSignature();
    cypBuilder.addAllSignature(sign21);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22 = sign(prikeys, hash);
    cypBuilder.clearSignature();
    cypBuilder.addAllSignature(sign21_22);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
    ...
    ArrayList<ByteString> sign21_22_23 = sign(prikeys, hash);
    cypBuilder.clearSignature();
    cypBuilder.addAllSignature(sign21_22_23);
    transactionCapsule = new TransactionCapsule(cypBuilder.build());
    try {
      boolean result = transactionCapsule.validateSignature(dbManager);
      Assert.assertTrue(result);
  ...
  @Test
  public void cypCapsuleClearTest() {
    Transaction tx = Transaction.newBuilder()
        .addRet(Result.newBuilder().setContractRet(contractResult.OUT_OF_TIME).build()).build();
    TransactionCapsule cypCap = new TransactionCapsule(tx);
    Result.contractResult contractResult = cypCap.getContractResult();
    cypCap.resetResult();
    Assert.assertEquals(cypCap.getInstance().getRetCount(), 0);
    cypCap.setResultCode(contractResult);
    Assert.assertEquals(cypCap.getInstance()
