...
import static org.cypher.protos.Protocol.Transaction.Contract.ContractType.ShieldedTransferContract;
import static org.cypher.protos.Protocol.Transaction.Contract.ContractType.TransferAssetContract;
...
public class BandwidthProcessor extends ResourceProcessor {
  ...
  @Override
  public void consume(TransactionCapsule cyp, TransactionTrace trace)
      throws ContractValidateException, AccountResourceInsufficientException,
      TooBigTransactionResultException {
    List<Contract> contracts = cyp.getInstance().getRawData().getContractList();
    if (cyp.getResultSerializedSize() > Constant.MAX_RESULT_SIZE_IN_TX * contracts.size()) {
    ...
    if (chainBaseManager.getDynamicPropertiesStore().supportVM()) {
      bytesSize = cyp.getInstance().toBuilder().clearRet().build().getSerializedSize();
    } else {
      bytesSize = cyp.getSerializedSize();
    }
    ...
    for (Contract contract : contracts) {
      ...
      logger.debug("cypId {}, bandwidth cost: {}", cyp.getTransactionId(), bytesSize);
  ...
  public long calculateGlobalNetLimit(AccountCapsule accountCapsule) {
    ...
    if (frozeBalance < CYP_PRECISION) {
      return 0;
    }
    long netWeight = frozeBalance / CYP_PRECISION;
