...
public class TriggerSmartContractServlet extends RateLimiterServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      TransactionCapsule cypCap = wallet
          .createTransactionCapsule(build.build(), ContractType.TriggerSmartContract);

      Transaction.Builder txBuilder = cypCap.getInstance().toBuilder();
      Transaction.raw.Builder rawBuilder = cypCap.getInstance().getRawData().toBuilder();
      ...
      Transaction cyp = wallet
          .triggerContract(build.build(), new TransactionCapsule(txBuilder.build()), cypExtBuilder,
              retBuilder);
      cyp = Util.setTransactionPermissionId(jsonObject, cyp);
    ...
    response.getWriter().println(Util.printTransactionExtention(cypExtBuilder.build(), visible));
  }
}
