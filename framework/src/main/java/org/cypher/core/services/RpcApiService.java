...
import org.cypher.api.DatabaseGrpc.DatabaseImplBase;
...
import org.cypher.api.GrpcAPI.DelegatedResourceMessage;
import org.cypher.api.GrpcAPI.DiversifierMessage;
import org.cypher.api.GrpcAPI.EasyTransferAssetByPrivateMessage;
import org.cypher.api.GrpcAPI.EasyTransferAssetMessage;
import org.cypher.api.GrpcAPI.EasyTransferByPrivateMessage;
import org.cypher.api.GrpcAPI.EasyTransferMessage;
...
import org.cypher.api.GrpcAPI.PaginatedMessage;
...
import org.cypher.api.GrpcAPI.TransactionIdList;
...
import org.cypher.api.MonitorGrpc;
import org.cypher.api.WalletExtensionGrpc;
import org.cypher.api.WalletGrpc.WalletImplBase;
import org.cypher.api.WalletSolidityGrpc.WalletSolidityImplBase;
...
import org.cypher.core.metrics.MetricsApiService;
import org.cypher.core.services.filter.LiteFnQueryGrpcInterceptor;
import org.cypher.core.services.ratelimiter.RateLimiterInterceptor;
import org.cypher.core.services.ratelimiter.RpcApiAccessInterceptor;
...
import org.cypher.protos.Protocol.NodeInfo;
...
import org.cypher.protos.contract.BalanceContract.AccountBalanceRequest;
import org.cypher.protos.contract.BalanceContract.AccountBalanceResponse;
...
public class RpcApiService implements Service {
  ...
  private void callContract(TriggerSmartContract request,
    ...
    try {
      TransactionCapsule cypCap = createTransactionCapsule(request,
          ContractType.TriggerSmartContract);
      Transaction cyp;
      if (isConstant) {
        cyp = wallet.triggerConstantContract(request, cypCap, cypExtBuilder, retBuilder);
      } else {
        cyp = wallet.triggerContract(request, cypCap, cypExtBuilder, retBuilder);
      }
      ...
      responseObserver.onNext(cypExtBuilder.build());
  ...
  private TransactionExtention transaction2Extention(Transaction transaction) {
    ...
    TransactionExtention.Builder cypExtBuilder = TransactionExtention.newBuilder();
    Return.Builder retBuilder = Return.newBuilder();
    cypExtBuilder.setTransaction(transaction);
    cypExtBuilder.setTxid(Sha256Hash.of(CommonParameter.getInstance()
        .isECKeyCryptoEngine(), transaction.getRawData().toByteArray()).getByteString());
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
    cypExtBuilder.setResult(retBuilder);
    return cypExtBuilder.build();
  }
  ...
  public class DatabaseApi extends DatabaseImplBase {
    ...
    @Override
    public void getBlockReference(org.cypher.api.GrpcAPI.EmptyMessage request,
        io.grpc.stub.StreamObserver<org.cypher.api.GrpcAPI.BlockReference> responseObserver) {
  ...
  public class WalletSolidityApi extends WalletSolidityImplBase {
    ...
    public void getDelegatedResourceAccountIndex(BytesMessage request,
        StreamObserver<org.cypher.protos.Protocol.DelegatedResourceAccountIndex> responseObserver) {
      responseObserver
    ...
    @Override
    public void getBurnCyp(EmptyMessage request, StreamObserver<NumberMessage> responseObserver) {
      getBurnCypCommon(request, responseObserver);
    }
  ...
  public class WalletApi extends WalletImplBase {
    ...
    private void createTransactionExtention(Message request, ContractType contractType,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule cyp = createTransactionCapsule(request, contractType);
        cypExtBuilder.setTransaction(cyp.getInstance());
        cypExtBuilder.setTxid(cyp.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (ContractValidateException e) {
        retBuilder.setResult(false).setCode(response_code.CONTRACT_VALIDATE_ERROR)
    ...
    public void getTransactionSign2(TransactionSign req,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule cyp = TransactionUtil.getTransactionSign(req);
        cypExtBuilder.setTransaction(cyp.getInstance());
        cypExtBuilder.setTxid(cyp.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (Exception e) {
    ...
    public void addSign(TransactionSign req,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule cyp = transactionUtil.addSign(req);
        cypExtBuilder.setTransaction(cyp.getInstance());
        cypExtBuilder.setTxid(cyp.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (Exception e) {
    ...
    private void checkVoteWitnessAccount(VoteWitnessContract req) {
      ...
      if (dbManager.getDynamicPropertiesStore().supportAllowNewResourceModel()) {
        Preconditions.checkArgument(account.getAllCypherPower() < votesCount,
            "cypher power[" + account.getAllCypherPower() + "] <  VotesCount[" + votesCount + "]");
      } else {
        Preconditions.checkArgument(account.getCypherPower() < votesCount,
            "cypher power[" + account.getCypherPower() + "] <  VotesCount[" + votesCount + "]");
      }
    ...
    public void listNodes(EmptyMessage request, StreamObserver<NodeList> responseObserver) {
      ...
      nodeHandlerMap.entrySet().stream()
          .forEach(v -> {
            org.cypher.common.overlay.discover.node.Node node = v.getValue().getNode();
    ...
    public void createShieldedTransaction(PrivateParameters request,
      ...
      try {
        ...
        TransactionCapsule cyp = wallet.createShieldedTransaction(request);
        cypExtBuilder.setTransaction(cyp.getInstance());
        cypExtBuilder.setTxid(cyp.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
    ...
    public void createShieldedTransactionWithoutSpendAuthSig(PrivateParametersWithoutAsk request,
      ...
      try {
        ...
        TransactionCapsule cyp = wallet.createShieldedTransactionWithoutSpendAuthSig(request);
        cypExtBuilder.setTransaction(cyp.getInstance());
        cypExtBuilder.setTxid(cyp.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (ContractValidateException | ZksnarkException e) {
    ...
    public void createShieldedContractParameters(
        ...
        StreamObserver<org.cypher.api.GrpcAPI.ShieldedTRC20Parameters> responseObserver) {
      try {
        checkSupportShieldedTRC20Transaction();
    ...
    public void scanShieldedTRC20NotesByIvk(
        ...
        StreamObserver<org.cypher.api.GrpcAPI.DecryptNotesTRC20> responseObserver) {
      long startNum = request.getStartBlockIndex();
      long endNum = request.getEndBlockIndex();
      try {
        checkSupportShieldedTRC20Transaction();
  ...
  public void getBurnCypCommon(EmptyMessage request,
      StreamObserver<NumberMessage> responseObserver) {
    try {
      long value = dbManager.getDynamicPropertiesStore().getBurnCypAmount();
      NumberMessage.Builder builder = NumberMessage.newBuilder();
      builder.setNum(value);
      responseObserver.onNext(builder.build());
    } catch (Exception e) {
