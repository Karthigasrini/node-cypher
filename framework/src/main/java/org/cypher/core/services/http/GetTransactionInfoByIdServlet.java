...
public class GetTransactionInfoByIdServlet extends RateLimiterServlet {
  ...
  private static String convertLogAddressToCypherAddress(TransactionInfo transactionInfo,
      boolean visible) {
    if (visible) {
      List<Log> newLogList = Util.convertLogAddressToCypherAddress(transactionInfo);
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      if (reply != null) {
        response.getWriter().println(convertLogAddressToCypherAddress(reply, visible));
      } else {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      if (reply != null) {
        response.getWriter().println(convertLogAddressToCypherAddress(reply, params.isVisible()));
      } else {
        response.getWriter().println("{}");
      }
    } catch (Exception e) {
