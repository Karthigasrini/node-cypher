...
import static org.cypher.common.utils.Commons.decodeFromBase58Check;
...
import org.cypher.core.actuator.TransactionFactory;
...
import org.cypher.core.services.http.JsonFormat.ParseException;
...
public class Util {
  ...
  public static byte[] generateContractAddress(Transaction cyp, byte[] ownerAddress) {
    ...
    byte[] txRawDataHash = Sha256Hash
        .of(CommonParameter.getInstance().isECKeyCryptoEngine(), cyp.getRawData().toByteArray())
        .getBytes();
  ...
  public static List<Log> convertLogAddressToCypherAddress(TransactionInfo transactionInfo) {
    ...
    for (Log log : transactionInfo.getLogList()) {
      ...
      if (oldAddress.length == 0 || oldAddress.length > 20) {
        ...
        logBuilder
            .setAddress(ByteString.copyFrom(TransactionTrace.convertToCypherAddress(newAddress)));
      }
