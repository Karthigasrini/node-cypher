package org.cypher.core.services.interfaceOnSolidity;
...
public class RpcApiServiceOnSolidity implements Service {
  ...
  private TransactionExtention transaction2Extention(Transaction transaction) {
    ...
    cypExtBuilder.setTxid(Sha256Hash.of(CommonParameter.getInstance().isECKeyCryptoEngine(),
  ...
  private class WalletSolidityApi extends WalletSolidityImplBase {
    ...
    public void getBurnCyp(EmptyMessage request, StreamObserver<NumberMessage> responseObserver) {
      walletOnSolidity.futureGet(
          () -> rpcApiService.getWalletSolidityApi().getBurnCyp(request, responseObserver)
      );
    }
