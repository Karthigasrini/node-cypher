...
import org.cypher.core.net.message.PbftCommitMessage;
...
@Slf4j(topic = "net")
@Component
public class FetchInvDataMsgHandler implements CypherMsgHandler {
  ...
  private void check(PeerConnection peer, FetchInvDataMessage fetchInvDataMsg) throws P2pException {
    ...
    if (type == MessageTypes.CYP) {
      for (Sha256Hash hash : fetchInvDataMsg.getHashList()) {
        if (peer.getAdvInvSpread().getIfPresent(new Item(hash, InventoryType.CYP)) == null) {
