...
import org.cypher.core.vm.repository.Type;
...
public class DepositImpl implements Deposit {
  ...
  public synchronized Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getCypHash())) {
      storage.generateAddrHash(contract.getCypHash());
    }
    return storage;
  }
  ...
  public synchronized void putStorageValue(byte[] address, DataWord key, DataWord value) {
    address = TransactionTrace.convertToCypherAddress(address);
  ...
  @Override
  public TransactionCapsule getTransaction(byte[] cypHash) {
    Key key = Key.create(cypHash);
    ...
    if (parent != null) {
      transactionCapsule = parent.getTransaction(cypHash);
    } else {
      try {
        transactionCapsule = getTransactionStore().get(cypHash);
