...
This is the description of  Google Protobuf implementation of Cypher's protocol.
...
- Cypher has 3 `types` of account: Normal, AssetIssue, Contract
...
- message `Account` has multiple attributes and 2 nested messages: 
  ...
  `balance`: the CYP balance of this account.

  `votes`: received votes of this account. – e.g. *{(“0x1b7w…9xj3”,323), (“0x8djq…j12m”,88),…,(“0x82nd…mx6i”,10001)}*.

  `asset`: other assets except CYP in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*.

  `assetV2`: other assets except CYP in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*. (used after allowing same name of token87)

  `frozen`: the freezed CYP of this account for receiving bandwidth
...
- message `TransactionRet`
  ...
  - message `ResourceReceipt`
    ...
    `net_fee`: consume yourself cyp of net usage.
  ...
  - Message `InternalTransaction`
    ...
    `callValueInfo`: Refers to asset transfer information in internal transactions, including cyp and trc10.
  ...
  - message `TransactionInfo`
   ...
   `unfreeze_amount`: unfreeze cyp amount.
...
- Cypher has 33 types of Contracts declared within [`Transaction`](#trans).
...
- There are 15 types of results while deploying contracts (refer to `Transaction.Result`):
  ...
  - message `VoteWitnessContract`
    ...
    - message `AssetIssueContract`
      ...
      `cyp_num`: cyp num defines token price.

      `precision`: precision.

      `num`: cyp num defines token price.
    ...
    - message `FreezeBalanceContract`
      ...
      `frozen_balance`: frozen amount of CYP.

      `frozen_duration`: frozen duration of CYP.

      `resource`: type of resource gained from freezing CYP.
    ...
    - message `UnfreezeBalanceContract`
     ...
     - message `TriggerSmartContract`
       ...
       `call_value`: CYP amount sent to smart contract.
...
- message `SmartContract`
  ...
  `call_value`: amount of CYP that send to the smart contract.
  ...
  `cyp_hash`:  transactionId of Deploying contract transaction.
