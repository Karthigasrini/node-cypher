...
public class InternalTransaction {
  ...
  /* the amount of cyp to transfer (calculated as sun) */
  private long value;
   ...
   */
  public InternalTransaction(Transaction cyp, InternalTransaction.CypType cypType)
      throws ContractValidateException {
    this.transaction = cyp;
    TransactionCapsule cypCap = new TransactionCapsule(cyp);
    this.protoEncoded = cypCap.getData();
    this.nonce = 0;
    ...
    this.deep = -1;
    if (cypType == CypType.CYP_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = WalletUtil.generateContractAddress(cyp);
      this.note = "create";
    ...
    } else if (cypType == CypType.CYP_CONTRACT_CALL_TYPE) {
    ...
    this.hash = cypCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a cyp callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. cyp or a token. can't be both. We should avoid using
  ...
  public enum CypType {
    CYP_PRECOMPILED_TYPE,
    CYP_CONTRACT_CREATION_TYPE,
    CYP_CONTRACT_CALL_TYPE,
    CYP_UNKNOWN_TYPE,
  }
