...
@Slf4j(topic = "DB")
@Component
public class DynamicPropertiesStore extends CypherStoreWithRevoking<BytesCapsule> {
  ...
  private static final byte[] BURN_CYP_AMOUNT = "BURN_CYP_AMOUNT".getBytes();
  ...
  private DynamicPropertiesStore(@Value("properties") String dbName) {
    ...
    try {
      ...
      this.saveWitnessPayPerBlock(400000L);
    }
    ...
    try {
      this.getTotalCypherPowerWeight();
    } catch (IllegalArgumentException e) {
      this.saveTotalCypherPowerWeight(0L);
    }
    ...
    try {
      ...
      this.saveCreateAccountFee(100_000L); // 0.1CYP
    }
  ...
  public void saveTotalCypherPowerWeight(long totalEnergyWeight) {
    this.put(DynamicResourceProperties.TOTAL_CYPHER_POWER_WEIGHT,
        new BytesCapsule(ByteArray.fromLong(totalEnergyWeight)));
  }

  public long getTotalCypherPowerWeight() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_CYPHER_POWER_WEIGHT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_CYPHER_POWER_WEIGHT"));
  }
  ...
  public void saveTotalStoragePool(long cyp) {
    this.put(TOTAL_STORAGE_POOL,
        new BytesCapsule(ByteArray.fromLong(cyp)));
  }
  ...
  public void saveTotalStorageTax(long cyp) {
  ...
  //The unit is cyp
  public void addTotalCypherPowerWeight(long amount) {
    long totalWeight = getTotalCypherPowerWeight();
    totalWeight += amount;
    saveTotalCypherPowerWeight(totalWeight);
  }
  ...
  public long getBurnCypAmount() {
    return Optional.ofNullable(getUnchecked(BURN_CYP_AMOUNT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(() -> new IllegalArgumentException("not found BURN_CYP_AMOUNT"));
  }

  public void burnCyp(long amount) {
    if (amount <= 0) {
      return;
    }
    amount += getBurnCypAmount();
    saveBurnCyp(amount);
  }

  private void saveBurnCyp(long amount) {
    this.put(BURN_CYP_AMOUNT, new BytesCapsule(ByteArray.fromLong(amount)));
  }
  ...
  private static class DynamicResourceProperties {
    ...
    private static final byte[] TOTAL_CYPHER_POWER_WEIGHT = "TOTAL_CYPHER_POWER_WEIGHT".getBytes();
