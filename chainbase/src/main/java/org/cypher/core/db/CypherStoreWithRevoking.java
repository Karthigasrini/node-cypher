...
import org.cypher.core.db.common.iterator.DBIterator;
import org.cypher.core.db2.common.DB;
import org.cypher.core.db2.common.IRevokingDB;
import org.cypher.core.db2.common.LevelDB;
import org.cypher.core.db2.common.RocksDB;
import org.cypher.core.db2.core.Chainbase;
import org.cypher.core.db2.core.ICypherChainBase;
import org.cypher.core.db2.core.RevokingDBWithCachingOldValue;
...
@Slf4j(topic = "DB")
public abstract class CypherStoreWithRevoking<T extends ProtoCapsule> implements ICypherChainBase<T> {
  ...
  protected CypherStoreWithRevoking(String dbName) {
  ...
  protected CypherStoreWithRevoking(DB<byte[], byte[]> db) {
    int dbVersion = CommonParameter.getInstance().getStorage().getDbVersion();
    if (dbVersion == 2) {
  ...
  // only for test
  protected CypherStoreWithRevoking(String dbName, RevokingDatabase revokingDatabase) {
    this.revokingDB = new RevokingDBWithCachingOldValue(dbName,
        (AbstractRevokingStore) revokingDatabase);
  }

  // only for test
  protected CypherStoreWithRevoking(String dbName, Options options,
