...
public class StorageMarket {
  ...
  private long exchangeToSupply(boolean isCYP, long quant) {
    logger.info("isCYP: " + isCYP);
    long balance = isCYP ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchangeToSupply2(boolean isCYP, long quant) {
    logger.info("isCYP: " + isCYP);
    long balance = isCYP ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isCYP, long supplyQuant) {
    long balance = isCYP ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isCYP) {
    long relay = exchangeToSupply(isCYP, from);
    return exchange_from_supply(!isCYP, relay);
  }
