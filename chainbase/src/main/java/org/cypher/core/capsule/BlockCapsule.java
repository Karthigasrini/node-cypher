...
import org.cypher.core.capsule.utils.MerkleTree;
...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public BlockCapsule(long timestamp, ByteString parentHash, long number,
    ...
    // block
    Block.Builder blockBuild = Block.newBuilder();
    transactionList.forEach(cyp -> blockBuild.addTransactions(cyp));
    this.block = blockBuild.setBlockHeader(blockHeader).build();
    initTxs();
  }
  ...
  public void addTransaction(TransactionCapsule pendingCyp) {
    this.block = this.block.toBuilder().addTransactions(pendingCyp.getInstance()).build();
    getTransactions().add(pendingCyp);
  }
  ...
  private void initTxs() {
    transactions = this.block.getTransactionsList().stream()
        .map(cyp -> new TransactionCapsule(cyp))
