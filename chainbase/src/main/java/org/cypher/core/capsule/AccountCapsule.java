...
import org.cypher.protos.Protocol.Account.AccountResource;
import org.cypher.protos.Protocol.Account.Builder;
...
import org.cypher.protos.Protocol.Permission.PermissionType;
...
public class AccountCapsule implements ProtoCapsule<Account>, Comparable<AccountCapsule> {
  ...
  public long getCypherPowerUsage() {
    if (this.account.getVotesList() != null) {
  ...
  public long getAllCypherPower() {
    if (account.getOldCypherPower() == -1) {
      return getCypherPowerFrozenBalance();
    } else if (account.getOldCypherPower() == 0) {
      return getCypherPower() + getCypherPowerFrozenBalance();
    } else {
      return account.getOldCypherPower() + getCypherPowerFrozenBalance();
    }
  }
  ...
  public boolean oldCypherPowerIsNotInitialized() {
    return this.account.getOldCypherPower() == 0;
  }

  public boolean oldCypherPowerIsInvalid() {
    return this.account.getOldCypherPower() == -1;
  }

  public void initializeOldCypherPower() {
    long value = getCypherPower();
    if (value == 0) {
      value = -1;
    }
    setInstance(getInstance().toBuilder()
        .setOldCypherPower(value)
        .build());
  }

  public void invalidateOldCypherPower() {
    setInstance(getInstance().toBuilder()
        .setOldCypherPower(-1)
        .build());
  }
  ...
  public void setOldCypherPower(long value) {
    setInstance(getInstance().toBuilder()
        .setOldCypherPower(value)
        .build());
  }

  public void setFrozenForCypherPower(long frozenBalance, long expireTime) {
    Frozen newFrozen = Frozen.newBuilder()
        .setFrozenBalance(frozenBalance)
        .setExpireTime(expireTime)
        .build();
    ...
    setInstance(getInstance().toBuilder()
        .setCypherPower(newFrozen)
        .build());
  }

  public long getCypherPowerFrozenBalance() {
    return this.account.getCypherPower().getFrozenBalance();
  }
