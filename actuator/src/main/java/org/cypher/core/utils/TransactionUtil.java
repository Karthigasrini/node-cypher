...
import org.cypher.api.GrpcAPI.TransactionSignWeight.Result;
...
public class TransactionUtil {
  ...
  public static TransactionCapsule getTransactionSign(TransactionSign transactionSign) {
    ...
    TransactionCapsule cyp = new TransactionCapsule(transactionSign.getTransaction());
    cyp.sign(privateKey);
    return cyp;
  }
  ...
  public TransactionCapsule addSign(TransactionSign transactionSign)
    ...
    byte[] privateKey = transactionSign.getPrivateKey().toByteArray();
    TransactionCapsule cyp = new TransactionCapsule(transactionSign.getTransaction());
    cyp.addSign(privateKey, chainBaseManager.getAccountStore());
    return cyp;
  }

  public TransactionSignWeight getTransactionSignWeight(Transaction cyp) {
    ...
    try {
      ...
      if (cyp.getSignatureCount() > 0) {
        ...
        long currentWeight = TransactionCapsule.checkWeight(permission, cyp.getSignatureList(),
            Sha256Hash.hash(CommonParameter.getInstance()
                .isECKeyCryptoEngine(), cyp.getRawData().toByteArray()), approveList);
