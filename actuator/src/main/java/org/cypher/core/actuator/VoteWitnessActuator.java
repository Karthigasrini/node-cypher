  ...
  public boolean validate() throws ContractValidateException {
    ...
    try {
      ...
      if (dynamicStore.supportAllowNewResourceModel()) {
        cypherPower = accountCapsule.getAllCypherPower();
      } else {
        cypherPower = accountCapsule.getCypherPower();
      }

      sum = LongMath
          .checkedMultiply(sum, CYP_PRECISION); //cyp -> drop. The vote count is based on CYP
      if (sum > cypherPower) {
        throw new ContractValidateException(
