...
public class UnfreezeBalanceActuator extends AbstractActuator {
  ...
  public boolean execute(Object result) throws ContractExeException {
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      switch (unfreezeBalanceContract.getResource()) {
        ...
        case ENERGY:
          ...
          break;
        case CYPHER_POWER:
          unfreezeBalance = accountCapsule.getCypherPowerFrozenBalance();
          accountCapsule.setInstance(accountCapsule.getInstance().toBuilder()
              .setBalance(oldBalance + unfreezeBalance)
              .clearCypherPower().build());
          break;
        default:
          //this should never happen
          break;
    ...
    switch (unfreezeBalanceContract.getResource()) {
      case BANDWIDTH:
        dynamicStore
            .addTotalNetWeight(-unfreezeBalance / CYP_PRECISION);
        break;
      case ENERGY:
        dynamicStore
            .addTotalEnergyWeight(-unfreezeBalance / CYP_PRECISION);
        break;
      case CYPHER_POWER:
        dynamicStore
            .addTotalCypherPowerWeight(-unfreezeBalance / CYP_PRECISION);
        break;
      default:
        //this should never happen
        break;
    ...
    if (dynamicStore.supportAllowNewResourceModel()
        && accountCapsule.oldCypherPowerIsInvalid()) {
      switch (unfreezeBalanceContract.getResource()) {
        case BANDWIDTH:
        case ENERGY:
    ...
    if (dynamicStore.supportAllowNewResourceModel()
        && !accountCapsule.oldCypherPowerIsInvalid()) {
      accountCapsule.invalidateOldCypherPower();
    }
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      switch (unfreezeBalanceContract.getResource()) {
        ...
        case CYPHER_POWER:
          if (dynamicStore.supportAllowNewResourceModel()) {
            Frozen frozenBalanceForCypherPower = accountCapsule.getInstance().getCypherPower();
            if (frozenBalanceForCypherPower.getFrozenBalance() <= 0) {
              throw new ContractValidateException("no frozenBalance(CypherPower)");
            }
            if (frozenBalanceForCypherPower.getExpireTime() > now) {
              throw new ContractValidateException("It's not time to unfreeze(CypherPower).");
            }
          } else {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy]");
