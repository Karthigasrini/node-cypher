...
import org.cypher.common.runtime.InternalTransaction.ExecutorType;
...
import org.cypher.core.vm.LogInfoTriggerParser;
...
import org.cypher.core.vm.program.Program.JVMStackOverFlowException;
import org.cypher.core.vm.program.Program.OutOfTimeException;
import org.cypher.core.vm.program.Program.TransferException;
import org.cypher.core.vm.program.ProgramPrecompile;
...
public class VMActuator implements Actuator2 {
  ...
  @Getter
  @Setter
  private InternalTransaction.CypType cypType;
  ...
  public void validate(Object object) throws ContractValidateException {
    ...
    cyp = context.getCypCap().getInstance();
    blockCap = context.getBlockCap();
    if (VMConfig.allowTvmFreeze() && context.getCypCap().getCypTrace() != null) {
      receipt = context.getCypCap().getCypTrace().getReceipt();
    }
    //Route Type
    ContractType contractType = this.cyp.getRawData().getContract(0).getType();
    //Prepare Repository
    ...
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        cypType = CypType.CYP_CONTRACT_CALL_TYPE;
        call();
        break;
      case ContractType.CreateSmartContract_VALUE:
        cypType = CypType.CYP_CONTRACT_CREATION_TYPE;
        create();
        break;
      default:
    ...
    try {
      if (program != null) {
        if (null != blockCap && blockCap.generatedByMyself && blockCap.hasWitnessSignature()
            && null != TransactionUtil.getContractRet(cyp)
            && contractResult.OUT_OF_TIME == TransactionUtil.getContractRet(cyp)) {
        ...
        if (CypType.CYP_CONTRACT_CREATION_TYPE == cypType && !result.isRevert()) {
  ...
  private void create()
    ...
    CreateSmartContract contract = ContractCapsule.getSmartContractFromTransaction(cyp);
    if (contract == null) {
    ...
    byte[] contractAddress = WalletUtil.generateContractAddress(cyp);
    ...
    try {
      long feeLimit = cyp.getRawData().getFeeLimit();
      ...
      rootInternalTx = new InternalTransaction(cyp, cypType);
      ...
      ProgramInvoke programInvoke = ProgramInvokeFactory
          .createProgramInvoke(CypType.CYP_CONTRACT_CREATION_TYPE, executorType, cyp,
      ...
      byte[] txId = TransactionUtil.getTransactionId(cyp).getBytes();
  ...
  private void call()
    ...
    TriggerSmartContract contract = ContractCapsule.getTriggerContractFromTransaction(cyp);
    if (contract == null) {
      return;
    }
    ...
    if (isNotEmpty(code)) {
      ...
      long vmShouldEndInUs = vmStartInUs + thisTxCPULimitInUs;
      ProgramInvoke programInvoke = ProgramInvokeFactory
          .createProgramInvoke(CypType.CYP_CONTRACT_CALL_TYPE, executorType, cyp,
              tokenValue, tokenId, blockCap.getInstance(), rootRepository, vmStartInUs,
              vmShouldEndInUs, energyLimit);
      if (isConstantCall) {
  ...
  private double getCpuLimitInUsRatio() {
    ...
    if (ExecutorType.ET_NORMAL_TYPE == executorType) {
      ...
      if (blockCap != null && blockCap.generatedByMyself
        ...
        if (cyp.getRet(0).getContractRet() == contractResult.OUT_OF_TIME) {
  ...
  public long getTotalEnergyLimitWithFixRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (Arrays.equals(creator.getAddress().toByteArray(), caller.getAddress().toByteArray())) {
      ...
      // which will cost the feeLimit CYP when the creator's frozen energy is 0.
