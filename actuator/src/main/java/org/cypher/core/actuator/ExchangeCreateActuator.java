...
import static org.cypher.core.config.Parameter.ChainSymbol.CYP_SYMBOL_BYTES;
...
public class ExchangeCreateActuator extends AbstractActuator {
  ...
  public boolean execute(Object object) throws ContractExeException {
    ...
    try {
      ...
      if (Arrays.equals(firstTokenID, CYP_SYMBOL_BYTES)) {
      ...
      if (Arrays.equals(secondTokenID, CYP_SYMBOL_BYTES)) {
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (dynamicStore.getAllowSameTokenName() == 1) {
      if (!Arrays.equals(firstTokenID, CYP_SYMBOL_BYTES) && !isNumber(firstTokenID)) {
        throw new ContractValidateException("first token id is not a valid number");
      }
      if (!Arrays.equals(secondTokenID, CYP_SYMBOL_BYTES) && !isNumber(secondTokenID)) {
