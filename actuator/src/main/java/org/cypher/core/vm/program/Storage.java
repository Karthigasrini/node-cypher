...
public class Storage {
  ...
  private static byte[] addrHash(byte[] address, byte[] cypHash) {
    if (ByteUtil.isNullOrZeroArray(cypHash)) {
      return Hash.sha3(address);
    }
    return Hash.sha3(ByteUtil.merge(address, cypHash));
  }

  public void generateAddrHash(byte[] cypId) {
    // update addreHash for create2
    addrHash = addrHash(address, cypId);
  }
