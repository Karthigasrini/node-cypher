package org.cypher.core.vm.program;
...
import org.cypher.core.exception.CypherException;
import org.cypher.core.utils.TransactionUtil;
import org.cypher.core.vm.EnergyCost;
import org.cypher.core.vm.MessageCall;
import org.cypher.core.vm.Op;
import org.cypher.core.vm.PrecompiledContracts;
import org.cypher.core.vm.VM;
import org.cypher.core.vm.VMConstant;
import org.cypher.core.vm.VMUtils;
import org.cypher.core.vm.config.VMConfig;
import org.cypher.core.vm.nativecontract.FreezeBalanceProcessor;
import org.cypher.core.vm.nativecontract.UnfreezeBalanceProcessor;
import org.cypher.core.vm.nativecontract.VoteWitnessProcessor;
import org.cypher.core.vm.nativecontract.WithdrawRewardProcessor;
import org.cypher.core.vm.nativecontract.param.FreezeBalanceParam;
...
import org.cypher.core.vm.program.invoke.ProgramInvoke;
import org.cypher.core.vm.program.invoke.ProgramInvokeFactory;
import org.cypher.core.vm.program.listener.CompositeProgramListener;
import org.cypher.core.vm.program.listener.ProgramListenerAware;
import org.cypher.core.vm.program.listener.ProgramStorageChangeListener;
import org.cypher.core.vm.repository.Repository;
import org.cypher.core.vm.trace.ProgramTrace;
import org.cypher.core.vm.trace.ProgramTraceListener;
import org.cypher.core.vm.utils.MUtil;
...
import org.cypher.protos.contract.Common;
import org.cypher.protos.contract.SmartContractOuterClass.SmartContract;
import org.cypher.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send CYP to.
   * @param value the CYP value transferred in the internal transaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] obtainer = obtainerAddress.toCypherAddress();
      ...
      try {
        ...
        if (VMConfig.allowTvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all cyp failed in suicide: %s", e.getMessage());
        }
  ...
  private void transferDelegatedResourceToInheritor(byte[] ownerAddr, byte[] inheritorAddr, Repository repo) {
    ...
    repo.addTotalNetWeight(-frozenBalanceForBandwidthOfOwner / CYP_PRECISION);

    long frozenBalanceForEnergyOfOwner =
        ownerCapsule.getAccountResource().getFrozenBalanceForEnergy().getFrozenBalance();
    repo.addTotalEnergyWeight(-frozenBalanceForEnergyOfOwner / CYP_PRECISION);
...
//    return freezeCheck && voteCheck;
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setCypHash(ByteString.copyFrom(rootTransactionId));
        }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = msg.getCodeAddress().toCypherAddress();
    byte[] senderAddress = getContextAddress();
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer cyp failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
...
//  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(address.toCypherAddress());
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] cypherAddr = address.toCypherAddress();
    AccountCapsule account = getContractState().getAccount(cypherAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(cypherAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(cypherAddr, contract);
        }
      } else {
  ...
  public byte[] getContextAddress() {
    return invoke.getContractAddress().toCypherAddress();
  }
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState().getBalance(address.toCypherAddress());
    return new DataWord(balance);
  }

  public DataWord getRewardBalance(DataWord address) {
    long rewardBalance = VoteRewardUtil.queryReward(address.toCypherAddress(), getContractState());
    return new DataWord(rewardBalance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState().getContract(address.toCypherAddress());
    return contract != null ? DataWord.ONE() : DataWord.ZERO();
  }

  public DataWord isSRCandidate(DataWord address) {
    WitnessCapsule witnessCapsule = getContractState().getWitness(address.toCypherAddress());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState().getTokenBalance(address.toCypherAddress(),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    ...
    if (VMConfig.allowTvmIstanbul()) {
      ...
      senderAddress = getCallerAddress().toCypherAddress();
    }
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (msg.getOpCode() == Op.CALLCODE || msg.getOpCode() == Op.DELEGATECALL) {
      ...
      contextAddress = msg.getCodeAddress().toCypherAddress();
    }
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      if (msg.getOpCode() == Op.DELEGATECALL) {
        contract.setCallerAddress(getCallerAddress().toCypherAddress());
      } else {
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowTvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer trc10 or CYP to non-exist address(would create one)
  ...
  public boolean freeze(DataWord receiverAddress, DataWord frozenBalance, DataWord resourceType) {
    ...
    byte[] owner = getContextAddress();
    byte[] receiver = receiverAddress.toCypherAddress();
  ...
  public long freezeExpireTime(DataWord targetAddress, DataWord resourceType) {
    byte[] owner = getContextAddress();
    byte[] target = targetAddress.toCypherAddress();
    ...
    try {
      ...
      for (int i = 0; i < witnessArrayLength; i++) {
        ...
        DataWord amount = new DataWord(Arrays.copyOfRange(amountArrayData,
            i * DataWord.WORD_SIZE, (i + 1) * DataWord.WORD_SIZE));
        param.addVote(witness.toCypherAddress(), amount.sValue().longValueExact());
      }
      if (internalTx != null) {
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(CypherException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(CypherException e) {
      return new PrecompiledContractException(e.getMessage());
    }
